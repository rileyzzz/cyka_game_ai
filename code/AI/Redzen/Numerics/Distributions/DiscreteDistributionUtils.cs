...
public static class DiscreteDistributionUtils
{
    ...
    public static bool SampleBernoulli(
        double probability,
        IRandomSource rng)
        // where T : struct, IBinaryFloatingPointIeee754<T>
    {
        return rng.NextUnitInterval<double>() < probability;
    }
    ...
    public static void SampleBernoulli(
        double probability,
        Span<bool> span,
        IRandomSource rng)
        // where T : struct, IBinaryFloatingPointIeee754<T>
    {
        for(int i=0; i < span.Length; i++)
            span[i] = rng.NextUnitInterval<double>() < probability;
    }
        ...
        IRandomSource rng)
    {
        ...
        Span<int> tmpSpan = new int[numberOfOutcomes];
