    ...
    where K : IComparable<K>
{
    ...
    public static void IntroSortInner(
        ...
        int depthLimit)
    {
        Sandbox.Diagnostics.Assert.True(!keys.IsEmpty);
        Sandbox.Diagnostics.Assert.True(vspan.Length == keys.Length);
    ...
    private static int PickPivotAndPartition(
        ...
        Span<W> wspan)
    {
        Sandbox.Diagnostics.Assert.True(keys.Length >= __introsortSizeThreshold);
