    ...
    where T : IComparable<T>
{
    ...
    {
        Sandbox.Diagnostics.Assert.True(n >= 0);
    ...
    public static int GallopLeft(
        ...
        int hint)
    {
        Sandbox.Diagnostics.Assert.True(len > 0 && hint >= 0 && hint < len);
        ...
        Sandbox.Diagnostics.Assert.True(lastOfs >= -1 && lastOfs < ofs && ofs <= len);
        ...
        Sandbox.Diagnostics.Assert.True(lastOfs == ofs);   // so a[baseIdx + ofs - 1] < key <= a[baseIdx + ofs]
        return ofs;
    }
