    ...
    where K : IComparable<K>
{
    ...
    {
        Sandbox.Diagnostics.Assert.True(_stackSize >= 2);
        Sandbox.Diagnostics.Assert.True(i >= 0);
        Sandbox.Diagnostics.Assert.True(i == _stackSize - 2 || i == _stackSize - 3);

        int base1 = _runBase[i];
        int len1 = _runLen[i];
        int base2 = _runBase[i + 1];
        int len2 = _runLen[i + 1];
        Sandbox.Diagnostics.Assert.True(len1 > 0 && len2 > 0);
        Sandbox.Diagnostics.Assert.True(base1 + len1 == base2);
        ...
        Sandbox.Diagnostics.Assert.True(k >= 0);
        base1 += k;
        len1 -= k;
        if(len1 == 0)
            return;
        ...
        Sandbox.Diagnostics.Assert.True(len2 >= 0);
        if(len2 == 0)
            return;
    ...
    {
        Sandbox.Diagnostics.Assert.True(len1 > 0 && len2 > 0 && base1 + len1 == base2);
        ...
        while(true)
        {
            ...
            do
            {
                Sandbox.Diagnostics.Assert.True(len1 > 1 && len2 > 0);
    ...
    outerExit:
        ...
        {
            Sandbox.Diagnostics.Assert.True(len2 == 0);
            Sandbox.Diagnostics.Assert.True(len1 > 1);
            tmp.Slice(cursor1, len1).CopyTo(s.Slice(dest));
            tmpv.Slice(cursor1, len1).CopyTo(v.Slice(dest));
        }
    }
    ...
    private void MergeHi(Span<K> s, Span<V> v, int base1, int len1, int base2, int len2)
    {
        ...
        while(true)
        {
            ...
            do
            {
                Sandbox.Diagnostics.Assert.True(len1 > 0 && len2 > 1);
    ...
    outerExit:
        ...
        if(len2 == 1)
        {
            Sandbox.Diagnostics.Assert.True(len1 > 0);
            dest -= len1;
            cursor1 -= len1;
            s.Slice(cursor1 + 1, len1).CopyTo(s.Slice(dest + 1));
        ...
        ref V[]? workv)
    {
        ...
        Sandbox.Diagnostics.Assert.True((work is null && workv is null) || (work is not null && workv is not null && work.Length == workv.Length));
        ...
        Sandbox.Diagnostics.Assert.True(lo == hi);
        ts.MergeForceCollapse(keys, vals);
        Sandbox.Diagnostics.Assert.True(ts._stackSize == 1);
    }
