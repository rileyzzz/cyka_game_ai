    ...
    where T : struct
{
        ...
        IRandomSource rng)
    {
        ...
        Sandbox.Diagnostics.Assert.True(pop.SpeciesArray!.Sum(x => x.Stats.TargetSizeInt) == pop.TargetSize);
    }
        ...
        IRandomSource rng)
    {
        ...
        if(stats.TargetSizeInt == 0)
        {
            Sandbox.Diagnostics.Assert.True(!isBestGenomeSpecies, "Zero target size assigned to specie that contains the best genome.");
