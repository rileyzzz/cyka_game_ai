...
public static class ConnectionSorter<T>
{
    ...
    public static void Sort(
        ...
        Span<T> vals)
    {
        Sandbox.Diagnostics.Assert.True(connIds.GetSourceIdSpan().Length == connIds.GetTargetIdSpan().Length);
        Sandbox.Diagnostics.Assert.True(connIds.GetSourceIdSpan().Length == vals.Length);
    ...
    private static void IntroSortInner(
        ...
        int depthLimit)
    {
        Sandbox.Diagnostics.Assert.True(!srcIds.IsEmpty);
        Sandbox.Diagnostics.Assert.True(depthLimit >= 0);
    ...
    private static int PickPivotAndPartition(
        ...
        Span<T> vals)
    {
        Sandbox.Diagnostics.Assert.True(srcIds.Length >= __introsortSizeThreshold);
    ...
    private static void Swap(
        ...
        int i, int j)
    {
        Sandbox.Diagnostics.Assert.True(i != j);
